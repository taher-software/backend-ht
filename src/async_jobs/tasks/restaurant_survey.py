...
def create_restaurant_survey_notif_title(language: str):
    """
    Generate restaurant survey notification title with Firestore caching.
    ...
    cached_doc = firestore_client.find_document(
        collection_name=CachingCollectionName.RESTAURANT_SURVEY_NOTIF_TITLES,
        ...
        messages=[
            ...
            },
            {
                ...
                "content": (
                    ...
                    '"🍽️ How Was Your Dining Experience Today?"'
                ),
            },
        ],
    )
...
def create_restaurant_survey_notif_body(guest_name: str, language: str):
    """
    Generate restaurant survey notification body in the specified language.
        ...
        messages=[
            {
                ...
                "content": (
                    ...
                    "for hotel guests in different languages. "
                    "The notification should ask the guest about their dining experience, "
            ...
            },
            {
                ...
                "content": (
                    ...
                    "Use this example as structure:\n"
                    '"Hello {guest_name}, how was your dining experience today? Please share your feedback in the restaurant survey in the app"'
                ),
            },
        ],
    )
...
def send_admin_failure_notification_for_job(details: dict):
    ...
    This is a backoff on_giveup handler.
    """
    try:
        ...
        try:
            # Get guest count for context
            guests_count = 0
            try:
                guests_ids = get_current_guest_for_given_namespace(
                    namespace_id, check_meal_eligibility=True, db=db
                )
...
def send_guest_restaurant_survey_notif(guest_id: str, db=None):
    """Send restaurant survey notification to a specific guest"""
    if db is None:
        raise ValueError("Database session must be provided")

    try:
        logger.info(f"Sending restaurant survey notification to guest {guest_id}")
        ...
        try:
            notif_title = create_restaurant_survey_notif_title(guest.pref_language)
            notif_body = create_restaurant_survey_notif_body(
            ...
            notif_title = "🍽️ How Was Your Dining Experience Today?"
            notif_body = f"Hello {guest.first_name}, how was your dining experience today? Please share your feedback in the restaurant survey in the app"

        # Send push notification
        try:
            send_push_notification(guest.current_device_token, notif_title, notif_body)
            logger.info(
                f"Successfully sent restaurant survey notification to guest {guest_id}"
            )
        except Exception as e:
            logger.error(
        ...
        logger.error(
            f"Error sending restaurant survey notification to guest {guest_id}: {str(e)}",
            exc_info=True,
        )
        raise


def process_namespace_restaurant_surveys(
    namespace_id: int, task_id: str, db=None
) -> dict:
    """
    Process restaurant survey notifications for a specific namespace.

    This extracts the core logic for sending restaurant survey notifications
    to all eligible guests in a single namespace.
    ...
    try:
        logger.info(
            f"[Task {task_id}] Processing restaurant surveys for namespace {namespace_id}"
        )

        # Get current guests for this namespace with meal eligibility check
        guests_ids = get_current_guest_for_given_namespace(
            namespace_id, check_meal_eligibility=True, db=db
        )
        ...
        if not guests_ids:
            logger.info(
                f"[Task {task_id}] No eligible guests found for namespace {namespace_id}"
            )
            return {
                "sent": 0,
                "guest_errors": 0,
        ...
        logger.info(
            f"[Task {task_id}] Found {len(guests_ids)} eligible guests in namespace {namespace_id}"
        )
        ...
        for guest_phone in guests_ids:
            try:
                ...
                send_guest_restaurant_survey_notif(guest_phone, db=db)
                sent_count += 1
                logger.info(
                    f"[Task {task_id}] Sent restaurant survey to guest {guest_phone} in namespace {namespace_id}"
                )

            except Exception as e:
                logger.error(
                    f"[Task {task_id}] Failed to send notification to guest {guest_phone} "
        ...
        if sent_count == 0 and guest_error_count > 0:
            logger.warning(
                f"[Task {task_id}] All  Guest notification attempts failed for namespace {namespace_id}"
            )
            raise Exception(
                f"All Guest notification attempts failed for namespace {namespace_id}"
            )
        logger.info(
            f"[Task {task_id}] Completed namespace {namespace_id}: "
        ...
        logger.error(
            f"[Task {task_id}] Critical error processing namespace {namespace_id}: {str(e)}",
            exc_info=True,
        )
        raise
...
@backoff.on_exception(
    ...
    max_tries=3,  # Total attempts (matches Celery's max_retries=2)
...
def send_notif_restaurant_survey_for_namespace(namespace_id: int, job_id: str):
    ...
    try:
        logger.info(
            f"[Job {task_id}] Processing restaurant surveys for namespace {namespace_id}"
        )

        # Process this namespace
        result = process_namespace_restaurant_surveys(namespace_id, task_id, db=db)
